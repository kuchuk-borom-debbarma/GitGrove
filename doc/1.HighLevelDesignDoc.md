...
# **GitGroove — High-Level Design (v1)**



# **1. Purpose**

GitGroove manages **multiple logical repositories inside a single Git repository**.
It organizes repos, builds hierarchy, and creates **real Git branches** for each logical repo using a namespace-based scheme.

GitGroove does NOT modify Git's core behavior — it simply structures the workspace.

---

# **2. Initialization (`gitgroove init`)**
...
All GitGroove files and metadata live inside `.gitgroove/` and are tracked on the `gitgroove` branch.
...
* **Invalid metadata (broken JSON):**
  GitGroove must stop if `.gitgroove` is corrupted.
...
* It will create a dedicated **repo main branch**:
  ...
  ```
  gitgroove/repos/<repo-name>/branches/main
  ```
...
* **User later moves folder manually:**
  GitGroove cannot detect this → user must update path explicitly.

* **Registering overlapping directories:**
  If repo paths overlap unintentionally, GitGroove must warn or block.
...
```
gitgroove link --parent X --child Y
```
...
* **Ambiguous parent selection:**
  If multiple possible parents exist based on path, GitGroove must require user confirmation.
...
```
gitgroove/repos/billing_repo/branches/main
```

For child repo:

```
gitgroove/repos/services/children/billing_repo/branches/main
```
...
* **Unsafe characters in branch names:**
  GitGroove must sanitize (e.g., slashes in user branches).
...
* **Deleting repo branches manually:**
  Could break GitGroove; system must detect missing namespaced branches and report clearly.
...
* Are stored in the namespace branch:
  ...
  ```
  gitgroove/repos/<repo>/branches/<branch>
  ```
...
* **Renames inside repo path:**
  Behave like normal Git — GitGroove imposes no special rules.
...
```
git merge gitgroove/repos/myrepo/branches/feature
     into
gitgroove/repos/myrepo/branches/main
```
...
* **Untracked files that would be overwritten:**
  GitGroove must detect and warn/block.
...
GitGroove v1 focuses purely on:
